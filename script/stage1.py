...
from dataset_stage1 import load_wow_dataset
...
def main():
    ...
    datasets = load_wow_dataset()

    def build_input_from_sentence(sentences, max_length):
    ...
    def prepare_features_100samples(examples):
        query_input_ids, query_attention_mask, query_token_type_ids = build_input_from_sentence(examples["query"], MAX_QUERY_LEN)
        knowledge_input_ids, knowledge_attention_mask, knowledge_token_type_ids = build_input_from_sentence(examples["knowledge"], MAX_KNOWLEDGE_LEN)
        response_input_ids, response_attention_mask, response_token_type_ids = build_input_from_sentence(examples["response"], MAX_RESPONSE_LEN)

        examples["query_input_ids"] = query_input_ids
        examples["knowledge_input_ids"] = knowledge_input_ids
        examples["response_input_ids"] = response_input_ids

        examples["query_attention_mask"] = query_attention_mask
        examples["knowledge_attention_mask"] = knowledge_attention_mask
        examples["response_attention_mask"] = response_attention_mask

        examples["query_token_type_ids"] = query_token_type_ids
        examples["knowledge_token_type_ids"] = knowledge_token_type_ids
        examples["response_token_type_ids"] = response_token_type_ids
        return examples
    ...
    def prepare_features(examples):
        ...
        GROUP_EXAMPLES = negative_samples + 1
        ...
        for id, group in ids2_group.items():
            ...
            for example_index in group:
                ...
                if labels[example_index] == 1:
                    if "positive_query_{}_{}".format("input_ids", pos_num) not in examples.keys():
                        examples["positive_query_{}_{}".format("input_ids", pos_num)] = [query_input_ids[example_index]]
                        examples["positive_knowledge_{}_{}".format("input_ids", pos_num)] = [knowledge_input_ids[example_index]]
                        examples["positive_response_query_{}_{}".format("input_ids", pos_num)] = [response_input_ids[example_index]]
                    else:
                        examples["positive_query_{}_{}".format("input_ids", pos_num)].append(query_input_ids[example_index])
                        examples["positive_knowledge_{}_{}".format("input_ids", pos_num)].append(knowledge_input_ids[example_index])
                        examples["positive_response_{}_{}".format("input_ids", pos_num)].append(response_input_ids[example_index])
    ...
    trainer = PtrTrainerPredictit(
        model=model,
        nagative_samples=negative_samples,
        args=training_args,
