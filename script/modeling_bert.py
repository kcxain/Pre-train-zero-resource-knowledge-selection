...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.segment_embeddings = nn.Embedding(3, config.hidden_size)
...
class BertForSequenceClassificationWithHingeLoss(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.num_labels == 1:
                #  We are doing regression
                loss = self.HingeLoss(logits.view(-1), labels.view(-1))
            else:
                loss_fct = CrossEntropyLoss()
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
...
class BertScore(BertPreTrainedModel):
    ...
    def _compute_bce_loss(self, all_scores):
        ...
        bce_loss = torch.nn.BCELoss(scores, labels)
        return bce_loss
